  ...
  return (
    <Dialog open={open} onOpenChange={() => {}}>
      <DialogContent className="max-w-2xl p-0 border-0 bg-transparent shadow-none [&>button]:hidden">
        <div className="relative overflow-hidden rounded-xl bg-gradient-subtle shadow-card transition-all duration-500">
          ...
          <div className="relative p-6">
            ...
            {userType && (
              <div className="animate-fade-in">
                <div className="p-4 rounded-lg bg-card/30 backdrop-blur-sm border border-primary/30 shadow-glow">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <RadioGroup value={subCategory} onValueChange={setSubCategory}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {(userType === "personal" ? personalOptions : professionalOptions).map((option) => (
                            <div key={option.value} className="flex items-center space-x-2 p-3 rounded-lg border border-border bg-card/50 hover:border-primary/50 hover:bg-card/70 transition-all duration-200 backdrop-blur-sm">
                              <RadioGroupItem 
                                value={option.value} 
                                id={option.value}
                      ...
                      {subCategory === "other" && (
                        <div className="space-y-2 pt-2 animate-fade-in">
                          <Label htmlFor="other-text" className="text-xs font-medium text-foreground">
                          ...
                          <Input
                            ...
                            className="bg-input/50 border-border focus:border-primary text-sm"
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex justify-center pt-4">
                      <Button 
                        ...
                        disabled={!canSubmit}
                        className="bg-gradient-primary hover:opacity-90 text-white px-8 py-2 rounded-lg font-medium transition-opacity disabled:opacity-50"
                      >
